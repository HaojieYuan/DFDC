...
#from datasetspreprocess.face_extraction import FaceExtraction
...
#对图片进行变换
    ...
    for epoch in range(opt.resume+1, opt.niter+1):
        ...
        for img_data, labels_data in tqdm(dataloader_train):
            ...
            loss_dis = network_loss(classes, labels_data.data) #计算loss
            ...
            tol_label = np.concatenate((tol_label, img_label)) #数组拼接
        ...
        acc_train = metrics.accuracy_score(tol_label, tol_pred) #计算准确度
        loss_train /= count
